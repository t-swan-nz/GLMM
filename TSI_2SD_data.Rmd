...
tab_model(model_fructose_concentration, show.stat = T, show.est = T, show.se = T, show.intercept = T, show.fstat = T)
...
Plot of fructose concentration by sex
```{r}
#overall figure
...
#Fig b
TSI_data_positive %>% ggplot(mapping = aes(x=sex, y=log(fructose_concentration), fill = time2)) + scale_fill_discrete(limits=positions_time) + geom_boxplot() + labs(x= "Sex", y = "log fructose concentration (ug/uL)", fill ="Time") + theme_cowplot()
```
...
male_fructose_concentration <- lmer(log(fructose_concentration) ~ time+habitat+flowers_binary + (1|day) + (1|station), data = TSI_male_positive)
...
plot(male_fructose_concentration)

Anova(male_fructose_concentration)
summ(male_fructose_concentration)

male_time_habitat <- emmeans(male_fructose_concentration, list(pairwise ~ time), adjust = "tukey")
male_time_habitat
#significantly higher fructose concentration in the morning than the afternoon for males. 
```

ggplot of fructose concentration for the males
```{r}

TSI_male_positive %>% ggplot(mapping = aes(x=time, y=log(fructose_concentration))) +geom_point() + geom_boxplot() + scale_x_discrete(limits=positions_time) + labs(x= "Time", y = "log Fructose concentration (ug/uL)") + theme_cowplot()
```
...
#log fructose concentration and add as a new column.I should try this.

female_fructose <- lmer(log(fructose_concentration) ~ time+habitat+flowers_binary + (1|day) + (1|station), data = TSI_female_positive)
...
Try to plot the two datasets on one graph
...
albo_female_with_negatives <- albo_data_cutoff %>% filter(sex == "Female") %>% ggplot(mapping = aes(x=sex, y=log(fructose_concentration)))+ geom_boxplot()+ geom_point(position = "jitter") + geom_point(data= female_negatives, position = "jitter", color = "orange")+ geom_hline(yintercept=log(0.001775502), lty="solid", col = "red")+ geom_hline(yintercept = log(0.004800524), lty="dotted", col = "red")+ geom_hline(yintercept = log(0.006951006), lty="longdash", col = "red") + geom_hline(yintercept = log(0.009538758), lty="twodash", col = "red") + labs(x= "", y = "Log fructose concentration (ug/uL)", fill ="Time") + theme_cowplot()
...
albo_male_with_negatives <- albo_data_cutoff %>% filter(sex == "Male") %>% ggplot(mapping = aes(x=sex, y=log(fructose_concentration)))+ geom_boxplot()+ geom_point(position = "jitter") + geom_point(data = male_negatives, position = "jitter", color = "orange") + geom_hline(yintercept=log(0.001430095), lty="solid", col = "red")+ geom_hline(yintercept = log(0.003445098), lty="dotted", col = "red")+ geom_hline(yintercept = log(0.0054601), lty="longdash", col = "red") + geom_hline(yintercept = log(0.007475102), lty="twodash", col = "red") + labs(x= "", y = "Log fructose concentration (ug/uL)", fill ="Time") + theme_cowplot() 
