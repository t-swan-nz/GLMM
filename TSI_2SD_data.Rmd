...
tab_model(albo_TSI_data_2SD_glmer) 
...
tab_model(male_only_2SD, show.stat = T, show.est = T, show.se = T, show.intercept = T, show.fstat = T)
...
# not logged female
albo_data_cutoff %>% ggplot(mapping = aes(x=sex, y=fructose_concentration)) + geom_point(position = "jitter") + geom_boxplot() + geom_hline(yintercept=0.001430095, lty="solid", col = "red")+ geom_hline(yintercept = 0.003445098
, lty="dotted", col = "red")+ geom_hline(yintercept = 0.0054601, lty="longdash", col = "red") + geom_hline(yintercept = 0.007475102, lty="twodash", col = "red") + labs(x= "Sex", y = "fructose concentration (ug/uL)", fill ="Time") + theme_cowplot()

#just try with the female data at the moment.
